...
public class Condition13 : FindVariables {
    public void UpdateTask(Math.Trio newABC, bool isActive) {
        ...
        Task.text = stringB + " = " + stringC + "k² - " + stringA + "l²";
    ...
    void FindParameters(int A, int B, int C, bool isActive) {
        ...
        for(int k = 0; k < 5000; k++) {
            for(int l = 0; l < 5000; l++) {
                if (B == C*Math.Pow(k, 2) - A*Math.Pow(l, 2)) ParametersList.Add(new Math.Pair(k, l));
            }
        }
